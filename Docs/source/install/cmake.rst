...
For example, this builds WarpX in 2D geometry and enables Python bindings and Nvidia GPU (CUDA) support:

.. code-block:: bash

   cmake -S . -B build -DWarpX_DIMS=2 -DWarpX_PYTHON=ON -DWarpX_COMPUTE=CUDA
...
.. code-block:: bash
   ...
   # find dependencies & configure
   cmake -S . -B build -DWarpX_PYTHON=ON
...
.. note::
   ...
   If your ``build/`` directory contains previously built Python modules, then ``--target pip_install`` picks them up as well.
...
.. code-block:: bash
   ...
   for d in 1 2 3 RZ; do
       cmake -S . -B build -DWarpX_DIMS=$d -DWarpX_PYTHON=ON
       cmake --build build -j 4
   done
